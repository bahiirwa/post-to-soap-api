...
function pms_post_published_notification( $post_id, $post ) {
    ...
    if( 'agent' === $post->post_type ) {
        ...
        if ( intval( $new_agent_id ) ) {
            ...
            $update_new_agent_id = update_post_meta( $post_id, 'agent_contact_details_agent_property24_id', $new_agent_id );

            if ( $update_new_agent_id ) {
                add_action( 'admin_notices', 'property24_admin_admin_notice' );
            }
...
function property24_admin_admin_notice() {
    ?>
